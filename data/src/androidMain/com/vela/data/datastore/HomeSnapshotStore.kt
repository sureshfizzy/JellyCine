...
class HomeSnapshotStore(
    ...
    private val homeSnapshotFileName = "VelaSnapshot.json"
    ...
    private fun readSnapshot(file: File): PersistedHomeSnapshot? {
        ...
        return runCatching {
            ...
            VelaJson.decodeFromString<PersistedHomeSnapshot>(rawJson)
        }.getOrElse {
            file.delete()
            null
        }
    ...
    private fun writeSnapshotAtomically(
        ...
        try {
            ...
            stream.write(VelaJson.encodeToString(snapshot).toByteArray(StandardCharsets.UTF_8))
            stream.flush()
